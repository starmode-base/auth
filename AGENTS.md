...
1. `packages/auth/src/contracts.ts` + `packages/auth2/src/contracts.ts` + `docs/spike/README-auth2.md`
2. `TODO.md`
3. `SPEC.md`

## Documentation map

- `packages/auth/src/contracts.ts` and `packages/auth/src/mechanisms/*-contracts.ts` — the promoted kernel, session, and OTP contracts. Win over README and code.
- `packages/auth2/src/contracts.ts` — the remaining passkey contract and re-exports of the promoted API. Wins over README and code.
...
- `docs/spike/` — design records moved unedited from the retired spike. The typed contracts win where they disagree. Folding them into the documentation is pending.
...
- Promotion phase: the kernel, opaque session, and OTP units live in `packages/auth`. The remaining passkey spike lives in `packages/auth2` and uses the promoted units. Spike units get tests once a second example uses them unchanged. Examples hand write bindings and client calls. Helpers come only from repetition observed across examples.
