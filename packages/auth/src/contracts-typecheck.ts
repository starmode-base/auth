...
import { makeAuth, makeOtpStrategy } from "./index";
import { makeAuth as contractMakeAuth } from "./contracts";

function expectType<T>(value: T): T {
  return value;
}

expectType<typeof contractMakeAuth>(makeAuth);
expectType<typeof makeAuth>(contractMakeAuth);
...
async function otpProbe(): Promise<void> {
  ...
  if (outcome.success) {
    ...
    expectType<"invalid_otp" | "authentication_disabled">(outcome.error);
  }
}
...
// @ts-expect-error The legacy object constructor is absent.
void makeAuth({ session, otp: otpEngine });

// @ts-expect-error Strategies do not receive session capabilities.
void kernel.refresh;

void auth.strategies.otp.authenticate({
  identifier: "person@example.com",
  otp: "123456",
  // @ts-expect-error Strategy inputs do not accept arbitrary authenticated identities.
  userId: "client-supplied",
});

declare const binarySession: SessionAdapter<SessionClaims, Uint8Array, {}>;
const binaryAuth = makeAuth(binarySession, reusableStrategies);
expectType<OtpStrategy<EmailOtpUser, Uint8Array>>(
  binaryAuth.strategies.emailOtp,
);

const infallibleAuth = makeAuth(binarySession, (strategyKernel) => ({
  custom: {
    authenticate: () =>
      strategyKernel.authenticate<AuthUser, never>(
        async (): Promise<Result<AuthUser, never>> => ({
          success: true,
          data: { userId: "custom-proof" },
        }),
      ),
  },
}));

async function infallibleProbe(): Promise<void> {
  const outcome = await infallibleAuth.strategies.custom.authenticate();
  expectType<Uint8Array>(outcome.data.session);
}
void infallibleProbe;
